...
4. **chown -R zhangke:root test/ 将文件夹下（包括test）的所有文件的用户改为zhangke, 用户组改为root**
## 2.1 chgrp 用户组
1. 用户组位于/etc/group下
2. **chgrp -R zhangke test 修改文件夹test下所有文件用户组改为zhangke**
...
# 十二、su 
1. (root) 用户下： 切换到用户zhangke
2. su zhangke 即可切换到zhangke用户下
...
**scp 指定文件 目的地**
### 6.1 本机文件拷贝到远端主机
1. 将本地131.zk文件拷贝到140下
1. scp ~/131.zk root@192.168.103.140:~
2. 会提示输入140的root用户密码
3. **scp -r 文件夹**  -> 拷贝文件夹
### 6.2 远端主机文件拷贝到本地
1. 将140上的文件140.zk拷贝到本机家目录~下
1. scp root@192.168.103.140:~/140.zk ~
2. 输入140密码
...
-》搜索结果如下
/var/log/t2stor/celery-13.log:101350:[2019-08-14 19:09:14,923: DEBUG/ForkPoolWorker-13] portal_stor.tasks.services_volume.protect_snap[2b2d597f-743c-47db-9df2-b6a2d5fa3125]: volume is_protect: Pool-1/zk02
/var/log/t2stor/celery-13.log:101363:[2019-08-14 19:09:15,217: INFO/ForkPoolWorker-13] portal_stor.tasks.services_volume.protect_snap[2b2d597f-743c-47db-9df2-b6a2d5fa3125]: volume is_protect success: Pool-1/zk02
```
